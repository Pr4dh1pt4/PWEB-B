...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->Cell(0,7,'SMA NEGERI 1 SUKOLILO',0,1,'C');
        
        $this->SetFont('Arial','',10);
        $this->Cell(0,5,'Daftar Siswa SMAN 1 SUKOLILO',0,1,'C');
    ...
    function Footer()
    {
        ...
        $this->Cell(0,10,'Halaman '.$this->PageNo().' - Data Database: sekolah',0,0,'C');
    }
}
...
$pdf->Cell($w[4], 8, 'NOMOR', 1, 0, 'C', true);
...
$query = mysqli_query($connect, "SELECT * FROM siswa ORDER BY nama_siswa ASC");
$no = 1;
$fill = false; 
...
while ($row = mysqli_fetch_array($query)){
    ...
    $pdf->Cell($w[4], 7, $row['nomor'], 1, 0, 'C', $fill);
